	...
	if(!client && stat == CONSCIOUS)
		...
		if(!(blinded > 0))
			for(var/mob/living/M in hearers(src))
				...
				if ((!a.lighting_use_dynamic || t.GetLightRange() >= 0.50) || (istype(src.glasses, /obj/item/clothing/glasses/night) || istype(src.glasses, /obj/item/clothing/glasses/thermal)))
